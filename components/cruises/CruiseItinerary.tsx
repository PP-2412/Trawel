  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-[#5b21b6] to-[#7c3aed] px-8 py-6 flex items-center justify-between">
          <div>
            <h2 className="font-display text-2xl font-semibold text-white">{selectedCruise.name}</h2>
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-100px)]">
          {/* Cruise Overview */}
          <div className="p-8 border-b border-[#ede9fe]">
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="flex-1">
                ...
                <p className="font-sans text-sm text-[#c9a227] font-medium mb-3">{selectedCruise.tagline}</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex items-center gap-2">
                    <Anchor className="w-4 h-4 text-[#5b21b6]" />
                    <span className="font-sans text-xs text-stone-600">Departs: {selectedCruise.departurePort}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-[#5b21b6]" />
                    <span className="font-sans text-xs text-stone-600">{selectedCruise.nights} Nights</span>
                  </div>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {selectedCruise.features.map((feature, idx) => (
                    <span key={idx} className="inline-flex items-center gap-1 px-2 py-1 bg-[#f5f3ff] rounded-full font-sans text-xs text-[#5b21b6]">
          ...
          {/* Itinerary Section */}
          <div className="p-8 border-b border-[#ede9fe]">
            <h3 className="font-display text-xl font-semibold text-[#2d1f4e] mb-6 flex items-center gap-2">
              <MapPin className="w-5 h-5 text-[#5b21b6]" />
              Cruise Itinerary
            </h3>

            {/* Itinerary Table */}
            <div className="bg-[#faf8ff] rounded-2xl overflow-hidden">
              {/* Table Header */}
              <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-gradient-to-r from-[#5b21b6] to-[#7c3aed]">
              ...
              {itinerary.map((day, index) => (
                <div
                  ...
                  className={`grid grid-cols-12 gap-4 px-6 py-4 items-center ${
                    index !== itinerary.length - 1 ? 'border-b border-[#ede9fe]' : ''
                  }`}
                >
                  <div className="col-span-1">
                    <span className="w-8 h-8 bg-[#5b21b6] text-white rounded-full text-xs flex items-center justify-center font-bold">
                      {day.day}
                    </span>
                  </div>
                  <div className="col-span-3">
                    <span className={`font-display text-base font-medium ${day.port === 'At Sea' ? 'text-[#5b21b6] italic' : 'text-[#2d1f4e]'}`}>
                      {day.port === 'At Sea' ? '🌊 ' : ''}{day.port}
                    </span>
                  </div>
                  <div className="col-span-2 text-center">
                    <span className="font-sans text-sm text-stone-600">{day.arrival}</span>
                  </div>
                  <div className="col-span-2 text-center">
                    <span className="font-sans text-sm text-stone-600">{day.departure}</span>
                  </div>
                  <div className="col-span-4">
                    <span className="font-sans text-xs text-stone-500">{day.description}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-8">
            <h3 className="font-display text-xl font-semibold text-[#2d1f4e] mb-6 flex items-center gap-2">
              ...
              <span className="font-sans text-xs font-normal text-stone-400 ml-2">(Per Person)</span>
            </h3>

            <div className="bg-[#faf8ff] rounded-2xl overflow-hidden border border-[#ede9fe]">
              {/* Budget Items */}
              {budgetItems.map((item, index) => (
                <div
                  key={item.label}
                  className={`flex items-center justify-between px-6 py-4 ${
                    index !== budgetItems.length - 1 ? 'border-b border-[#ede9fe]' : ''
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className="text-xl">{item.emoji}</span>
                    <span className="font-sans text-sm text-[#2d1f4e]">{item.label}</span>
                  </div>
                  <span className="font-sans text-sm font-semibold text-stone-400">TBD</span>
                </div>
              ))}

              {/* Total Cost */}
              <div className="flex items-center justify-between px-6 py-5 bg-gradient-to-r from-[#5b21b6] to-[#7c3aed]">
                <div className="flex items-center gap-3">
                  <Lock className="w-5 h-5 text-white" />
                  <span className="font-display text-lg font-semibold text-white">Total Cruise Cost</span>
                </div>
                <div className="text-right">
                  <span className="font-sans text-sm text-white/70">From </span>
                  <span className="font-sans text-lg font-bold text-[#fbbf24]">${selectedCruise.price}</span>
                </div>
              </div>
            </div>

            {/* Note */}
            <p className="font-sans text-xs text-stone-400 mt-4 text-center">
          ...
          <div className="p-8 pt-0">
            <button className="w-full bg-gradient-to-r from-[#c9a227] to-[#d4af37] text-[#2d1f4e] font-sans text-sm font-semibold tracking-wider uppercase py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-[1.02]">
